    ...
    return (
        <div className="paginationbutton">
            ...
            <ul className="pagination pagination-md justify-content-end">
                <li className="page-item"><button className="btn btn-light btn-sm" disabled={pages === 1} onClick={() => changingpages(1)}>&laquo;</button></li>
                <li className="page-item"><button className="btn btn-light btn-sm" disabled={pages === 1} onClick={() => changingpages(pages - 1)}>&lsaquo;</button></li>
                <li className="col-13 mt-1 ms-2 flaot-start">
                    <span className="float-start">Page</span>
                    <input className="form-control form-control-sm mm-5 col-5 ms-1 me-1 float-start"
                        type="text"
                        defaultValue={pages}
                        onChange={e => {
                ...
                {getPageNumbers().map((pageNumber, index) => (
                    <li key={index} className={pageNumber === pages ? 'btn btn-light btn-sm me-1 active' : 'btn btn-light btn-sm me-1'} onClick={() => changingpage(pageNumber)}>
                        {pageNumber}
                    </li>
                ))}
                ...
                <li className="page-item"><button className="btn btn-light btn-sm" disabled={pages === (Math.ceil(totalRows / perPage))} onClick={() => changingpages(pages + 1)}>&rsaquo;</button></li>
